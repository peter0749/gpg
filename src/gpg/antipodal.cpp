...
//  std::cout << "left_idx_viable: " << left_idx_viable.size() << ", right_idx_viable: " << right_idx_viable.size() << "\n";
  ...
  if (left_idx_viable.size() > 0 && right_idx_viable.size() > 0)
  {
    ...
    double top_viable_y = std::min(left_pts_viable.row(forward_axis).maxCoeff(), right_pts_viable.row(forward_axis).maxCoeff());
    double bottom_viable_y = std::max(left_pts_viable.row(forward_axis).minCoeff(), right_pts_viable.row(forward_axis).minCoeff());

    double top_viable_z = std::min(left_pts_viable.row(vertical_axis).maxCoeff(), right_pts_viable.row(vertical_axis).maxCoeff());
    double bottom_viable_z = std::max(left_pts_viable.row(vertical_axis).minCoeff(), right_pts_viable.row(vertical_axis).minCoeff());
